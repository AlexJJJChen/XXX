...
def iterated_execute_sql(predicted_sql,ground_truth,db_path,iterate_num):
    ...
    # print('111:',predicted_sql,predicted_res, '222:',ground_truth,ground_truth_res)
    time_ratio = 0
...
def package_sqls(sql_path, diff_json_path, db_root_path, mode='gpt', data_mode='dev'):
    clean_sqls = []
    db_path_list = []
    if mode == 'gpt':
        sql_data = json.load(open(sql_path, 'r'))
        database_name = json.load(open(diff_json_path, 'r'))
        # print(database_name[1])
        sql_data={key: sql_data[key] for key in sorted(sql_data.keys())}
        # print(sql_data)
        for idx, sql_str in sql_data.items():
            if type(sql_str) == str:
                sql=sql_str
                for item in database_name:
                    # print(idx,item['question_id']==int(idx))
                    if item['question_id']==int(idx):
                        db_name = item['db_id']
                        # sql=item['SQL']
                        # print(db_name)
                        break
...
def compute_ves_by_diff(exec_results,diff_json_path):
    ...
    for i,content in enumerate(contents[:len(exec_results)]):
...
def print_data(score_lists,count_lists):
    ...
    print('=========================================    VES   ========================================')
    print("                     Simple","            Moderate","          Challenging","             Total")
...
if __name__ == '__main__':
    ...
    pred_queries, db_paths = package_sqls(args.predicted_sql_path, args.diff_json_path, args.db_root_path, mode=args.mode_predict,
                                          data_mode=args.data_mode)
    # generate gt sqls:
    gt_queries, db_paths_gt = package_sqls(args.ground_truth_path, args.diff_json_path, args.db_root_path, mode='gt',
                                           data_mode=args.data_mode)
