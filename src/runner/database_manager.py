...
class DatabaseManager:
    ...
    def set_vector_db(self) -> str:
        ...
        if self.vector_db is None:
            try:
                #### 使用 ada-003 的话请修改下面的 f"{self.db_mode}_databasesada" 
                vector_db_path = DB_ROOT_PATH / f"{self.db_mode}_databases" / self.db_id/"context_vector_db" #self.db_directory_path / "context_vector_db"
    ...
    def get_database_schema_string(self, tentative_schema: Dict[str, List[str]], 
        ...
        schema_generator = DatabaseSchemaGenerator(
            ...
            #schema_with_task=schema_with_task,
            db_id=self.db_id,
            db_path=self.db_path,
        )
